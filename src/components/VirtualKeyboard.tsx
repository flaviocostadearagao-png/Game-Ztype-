...
export const VirtualKeyboard: React.FC<VirtualKeyboardProps> = ({
  ...
  return (
    <div className="w-full shrink-0 z-20 bg-slate-950/95 border-t-2 sm:border-t-4 border-slate-700 p-1 sm:p-2 shadow-2xl font-pixel select-none backdrop-blur-md animate-slideUp">
      <div className="max-w-3xl mx-auto flex flex-col gap-0.5 sm:gap-1.5">
        {/* Top Control Bar */}
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-1">
            <span className="text-[8px] sm:text-[10px] text-yellow-400 font-bold tracking-wide">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="mc-btn px-1.5 py-0.5 text-[8px] sm:text-[10px] text-emerald-300 flex items-center gap-1 hover:border-emerald-400"
            ...
            <button
              type="button"
              onClick={onClose}
              className="mc-btn p-0.5 sm:p-1 text-slate-300 hover:text-red-400"
        ...
        <div className="grid grid-cols-4 gap-1 bg-black/70 p-0.5 sm:p-1 border border-slate-800">
          <button
            type="button"
            onClick={() => onTriggerPowerup('tnt_nuke')}
            disabled={powerups.tnt_nuke <= 0}
            className={`mc-btn py-0.5 sm:py-1 text-[8px] sm:text-[10px] flex items-center justify-center gap-1 ${
              powerups.tnt_nuke > 0 ? 'text-red-400 border-red-900' : 'opacity-40'
            }`}
          >
            <Bomb className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-red-500" />
          ...
          >
            <Zap className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-red-500" />
        ...
        <div className="flex justify-center gap-0.5 sm:gap-1 w-full">
          {ROW_1.map((char) => (
            <button
              ...
              className="mc-btn flex-1 h-8 sm:h-10 text-[11px] sm:text-xs font-bold text-white hover:text-yellow-300 active:scale-95 transition-transform flex items-center justify-center min-w-[22px] max-w-[42px]"
            >
              {char}
            </button>
          ))}
        ...
        <div className="flex justify-center gap-0.5 sm:gap-1 w-full">
          ...
          <button
            ...
            className="mc-btn flex-[1.4] h-8 sm:h-10 text-xs text-red-300 hover:text-red-200 active:scale-95 transition-transform flex items-center justify-center gap-1 min-w-[36px]"
        ...
        <div className="flex justify-center gap-0.5 sm:gap-1 w-full overflow-x-auto">
          {ROW_ACCENTS.map((char) => (
            <button
              key={`key_${char}`}
              type="button"
              onClick={() => handleKeyClick(char)}
              className="mc-btn px-1 sm:px-2 h-7 sm:h-8 text-[10px] sm:text-[11px] font-bold text-emerald-300 hover:text-yellow-300 active:scale-95 transition-transform flex items-center justify-center min-w-[22px]"
            >
              {char}
            </button>
          ))}
          <button
            ...
            className="mc-btn px-2.5 h-7 sm:h-8 text-[10px] sm:text-[11px] text-slate-200 flex items-center justify-center gap-1"
          >
